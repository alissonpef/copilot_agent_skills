...
**Examples in this repo:** `/review`, `/test`, `/plan`.
...
**On Copilot Agent mode, use the built-in `Explore` subagent** rather than defining a custom research persona. `Explore` runs on Haiku, is denied write/edit tools, and is purpose-built for this pattern. Define a custom research subagent only when `Explore` doesn't fit (e.g. you need a domain-specific system prompt the model wouldn't infer).

---

## Copilot Agent mode compatibility

This catalog is harness-agnostic, but most readers will run it on Copilot Agent mode. Here's how each pattern maps onto Copilot Agent mode's primitives — and where the platform enforces our rules for us.

### Workspace customizations

Copilot discovers the repository customizations from `.github/`:

- `.github/copilot-instructions.md` - always-on project instructions
- `.github/prompts/` - slash commands and reusable task prompts
- `.github/agents/` - custom personas
- `.github/skills/` - reusable skills

Prompt files can reference custom agents with the `agent` field, and skills provide the detailed workflows those prompts and agents rely on. Keep the command names, agent names, and skill names aligned with the files they point to.

### Subagents vs. Agent Teams

Copilot Agent mode has two parallelism primitives. Pattern 3 (parallel fan-out with merge) maps to **subagents**. If you need teammates that talk to each other, use **Agent Teams** instead.
...
| Status | Stable | Experimental — requires `COPILOT_EXPERIMENTAL_AGENT_TEAMS=1` |
| Cost | Lower | Higher — each teammate is a separate agent instance |
...
Two rules in this catalog aren't just convention — Copilot Agent mode enforces them:

- **"Subagents cannot spawn other subagents"** (verbatim from the docs). Anti-pattern B (persona-calls-persona) and Anti-pattern D (deep persona trees) cannot exist on Copilot Agent mode by construction.
...
Custom agents in `.github/agents/` use the `.agent.md` format documented by VS Code, and prompt files in `.github/prompts/` can target a custom agent via the `agent` frontmatter field. If you need a new slash command, create a prompt file instead of overloading a persona file.

### Spawning multiple subagents in parallel

In Copilot Agent mode, parallel fan-out (Pattern 3) requires issuing **multiple Agent tool calls in a single assistant turn**. Sequential turns serialize execution. `/ship` calls this out explicitly. Any new orchestrator command should do the same.
...
Agent Teams is experimental. In `~/.github/settings.json`:
...
Requires Copilot Agent mode v2.1.32 or later. The personas in this repo are picked up automatically — no team-config files to author by hand.
...
- Replicates the work that slash commands and the existing workspace instructions already do

**What to do instead:** add or refine slash commands. Document intent → command mapping in `.github/copilot-instructions.md` and the repository README.
...
**What to do instead:** keep the user as the orchestrator. Document the recommended sequence in the repository README and let users invoke it.
